...
class ContainerImpl implements Container {
    ...
    void addInjectors(Class clazz, List<Injector> injectors) {
        ...
        addInjectorsForMethods(containerAsLastMethod(clazz), false, injectors);
    }

    // hack to reproduce WW-4827
    private Method[] containerAsLastMethod(Class clazz) {
      List<Method> q = null;
      if (clazz == ObjectFactory.class) {
          Method[] m = clazz.getDeclaredMethods();
          if (m != null) {
              System.out.println("moving setContainer method to the last element of the array");
              try {
                  Method stcm = clazz.getDeclaredMethod("setContainer", Container.class);
                  q = new ArrayList<Method>();
                  for (int i = 0; i < m.length; i++) {
                      if (!"setContainer".equals(m[i].getName())) {
                          q.add(m[i]);
                      }
                  }
                  q.add(stcm);
      ...
      return q == null ? clazz.getDeclaredMethods() : q.toArray(new Method[q.size()]);
    }
